...
│   Prototype Generator — wireframe từ feature list           │
...
input_sources:
  - type: pdf
    path: project-details-attachments/RFP-ACME-HR.pdf
    pages: 12
    received_at: 2026-04-26T09:30:00Z
  - type: docx
    path: project-details-attachments/requirements.docx
    received_at: 2026-04-26T09:35:00Z
  - type: csv
    path: project-details-attachments/employee-data-sample.csv
    received_at: 2026-04-26T09:40:00Z
  - type: image
    path: project-details-attachments/system-diagram.png
...
- RFP đính kèm: project-details-attachments/RFP-ACME-HR.pdf
- Requirements doc: project-details-attachments/requirements.docx
- Sample data: project-details-attachments/employee-data-sample.csv
- System diagram: project-details-attachments/system-diagram.png

> **Note:** File upload qua chat UI chỉ là entry point — source thực tế của tất cả input files luôn resolve về `project-details-attachments/[tên file].[format]`.
```
...
- **Attachment path convention:** Tất cả file input (PDF, DOCX, CSV, image) sau khi upload được lưu tại `project-details-attachments/[tên file].[format]`. Upload qua chat UI chỉ là entry point — path trong YAML `input_sources` và body `## Reference & attachments` luôn reference folder này.
...
output_format: "HTML & PNG"   # html_wireframe + png; UI chỉ show HTML

screens:
  - id: SCR-001
    name: Login / Authentication
    features: [F-001.1, F-001.2]
    path: prototype/SCR-001-login.html
  - id: SCR-002
    name: Employee Profile Management
    features: [F-002.1, F-002.2, F-002.3]
    path: prototype/SCR-002-employee-profile.html
  - id: SCR-003
    name: Time Attendance Dashboard
    features: [F-003.1, F-003.2]
    path: prototype/SCR-003-attendance.html
  - id: SCR-004
    name: Organization Chart
    features: [F-002.3]
    path: prototype/SCR-004-org-chart.html
---
...
> Wireframe: xem file `prototype/SCR-001-login.html`

## SCR-002: Employee Profile

> Wireframe: xem file `prototype/SCR-002-employee-profile.html`
...
- AI tự generate wireframe HTML từ feature list của từng màn hình.
- Output format: **HTML wireframe + PNG**. UI chỉ show HTML wireframe (không render PNG trên UI — PNG dùng cho export/client delivery).
- Có thể upload ảnh mockup từ client thay thế AI-generated screen.
...
- AC-1.1.4: Sau khi upload, tất cả file được move vào `project-details-attachments/`. Path trong brief.md YAML (`input_sources[].path`) và body (`## Reference & attachments`) luôn reference folder này.
...
3. Generate HTML wireframe per screen
4. Export PNG per screen
5. Write prototype/SCR-NNN.html per screen + index prototype.md
```
...
- AC-1.6.4: Output format: HTML wireframe + PNG. UI chỉ render HTML.
...
└── Project: ACME ERP HR  (created by Sales/Presale → full M1-M7 tabs visible)
    └── Presale (M1)
        ...
        │
        ├── Prototype
        │   ├── Screen list (từ feature list)
        │   ├── HTML wireframe viewer per screen
...
> **Note về Visibility Folder:** Khi presale click "Next → M2", toàn bộ artifacts được đẩy vào **Visibility Folder** (right panel) dưới dạng `.md` — show full ra để review trước khi advance. Export on-demand qua **sub-menu Project Details** theo format sau:
>
> | Artifact  | Visibility Folder | Project Details  |
> | --------- | ----------------- | ---------------- |
> | Brief     | `.md`             | `.docx`          |
> | Estimate  | `.md`             | `.xlsx`          |
> | WBS       | `.md`             | `.xlsx`          |
> | Technical | `.md`             | `.xlsx`          |
> | Prototype | `.md`             | `HTML + PNG`     |
...
POST   /workspaces/{id}/m1/advance                     # Advance M1 → M2: push .md artifacts vào Visibility Folder;
                                                        #   Project Details generate on-demand: Brief→docx, Estimate→xlsx,
                                                        #   WBS→xlsx, Technical→xlsx, Prototype→HTML+PNG; phase change M1→M2
...
| D9  | File export mặc định là `.md`               | Không auto-export DOCX/PDF; export qua Project Details  | §3               |
...
| D13 | Prototype output: HTML + PNG                | UI chỉ show HTML; PNG cho export/delivery               | §3.5.2           |
| D14 | Artifacts push vào Visibility Folder        | Visibility Folder=`.md` per artifact; Project Details export: Brief→docx, Estimate→xlsx, WBS→xlsx, Technical→xlsx, Prototype→HTML+PNG | §2.3 Step 8 |
...
| P5  | Documents delivery channel         | Visibility Folder (.md) + export via Project Details                  | **[TBD — pending confirm]** |
...
- **Visibility Folder** — right panel hiển thị toàn bộ artifact `.md` sau khi presale finish luồng M1. Export sang format khác (Brief→docx, Estimate→xlsx, WBS→xlsx, Technical→xlsx, Prototype→HTML+PNG) on-demand qua sub-menu Project Details.
